(function() {
  jQuery(function() {
    var loadCounter, poem, poemKeysPress, totalKeys;
    $("#saved").hide();
    $("#manifesto").draggable();
    loadCounter = 0;
    $(document).on('click', '#saved a', function(e) {
      var loading;
      e.preventDefault();
      $("#loader").append("<span class='green' id='loading'>LOADING...</span><br>");
      return loading = setInterval(function() {
        loadCounter += 1;
    ...
    return window.addEventListener("keypress", function(e) {
      ...
      if (totalKeys % 3 === 0) {
        ...
        if (poemKeysPress < maxPressed) {
          ...
          return console.log(poem[poemKeysPress].length);
